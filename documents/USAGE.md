...
MCP Manager is a CLI tool for managing which MCP servers are enabled in `~/.claude/.mcp-servers.json`, with support for project-local profiles.
...
Disables one or more MCP servers. Mirror of `enable`.
...
| `.mcp-servers.json` | all commands | The global MCP server configuration file at `~/.claude/.mcp-servers.json`. The tool only ever modifies the `enabled` field of each entry — it never changes `command`, `args`, `cwd`, `timeout`, or `env`. |
...
| `disable <server>...` | `disable` | Sets `"enabled": false` for one or more named servers. Multiple names can be passed in a single command. Pass `*` (quoted) to disable every server at once: `mcp-manager disable "*"`. Already-disabled servers are reported but not treated as errors. |
| `enable <server>...` | `enable` | Sets `"enabled": true` for one or more named servers. Multiple names can be passed in a single command. Already-enabled servers are reported but not treated as errors. |
